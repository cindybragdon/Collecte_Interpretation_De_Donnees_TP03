...
export async function fetchUsersFromAPi() {
  ...
  const usersFromApi = await fetch("https://fakestoreapi.com/users").then(
  ...
  const users = userList.map((user: UsersModel) => {
    ...
    const role: "gestionnaire" | "employe" =
      user.id % 3 === 0 ? roleGestionnaire : roleEmploye;
  ...
  try {
    ...
    console.log(
      "FETCH USER : Le fichier usersData.json est populé par API FakeStore/users"
    );
  } catch (err) {
    console.error(
